...
// 1.19 최신화
   ...
   final String SELECTALL_CATEGORY_PRICEASC = "SELECT P_NO ,P_NM, CATEGORY, P_DETAIL, P_IMG_URL, P_IMG_URL2, "
         + " P_AMT,DC_PERCENT, PRICE*((100-PRODUCT.DC_PERCENT)/100) AS PRICE FROM PRODUCT WHERE CATEGORY= ? AND PRICE BETWEEN ? AND ? ORDER BY PRICE ASC"; 
   // 상품 카테고리별  낮은가격순
   ...
   final String SELECTALL_CATEGORY_REGIDESC = "SELECT * FROM PRODUCT WHERE CATEGORY= ? AND PRICE BETWEEN ? AND ? ORDER BY P_NO DESC"; 
   	// 상품 카테고리별 최신순
